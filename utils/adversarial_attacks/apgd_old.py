...
def dlr_loss(self, x, y):
    ...
    return -(x[np.arange(x.shape[0]), y] - x_sorted[:, -2] *
             ind - x_sorted[:, -1] * (1. - ind)) / (
                 x_sorted[:, -1] - x_sorted[:, -3] + 1e-12)


class APGDAttack(AdversarialAttack):
    def __init__(self, model, num_classes, eps, n_iter=100, norm='Linf',
                 n_restarts=1, seed=0, loss='ce', eot_iter=1,
    ...
    def attack_single_run(self, x_in, y_in, targeted):
        ...
        # if len(y_in.shape) == 1 else y_in.clone().unsqueeze(0)
        y = y_in.clone()

        self.n_iter_2 = max(int(0.22 * self.n_iter), 1)
        self.n_iter_min = max(int(0.06 * self.n_iter), 1)
        self.size_dec = max(int(0.03 * self.n_iter), 1)
        if self.verbose:
        ...
        step_size = self.eps * torch.ones(
            [x.shape[0], 1, 1, 1]).to(x.device).detach() * torch.Tensor(
        ...
        reduced_last_check = \
            np.zeros(loss_best.shape) == np.zeros(loss_best.shape)
        # n_reduced = 0
                ...
                if self.norm in ['inf', 'linf', 'Linf']:
                    ...
                    x_adv_1 = torch.clamp(
                        torch.min(torch.max(x_adv_1, x - self.eps),
                                  x + self.eps), 0.0, 1.0)
                    x_adv_1 = torch.clamp(
                        torch.min(torch.max(x_adv + (x_adv_1 - x_adv) * a +
                                            grad2 * (1 - a), x - self.eps),
                                  x + self.eps), 0.0, 1.0)

                elif self.norm in ['l2', 'L2']:
                    x_adv_1 = x_adv + step_size * grad / (
                        (grad ** 2).sum(dim=(1, 2, 3),
                                        keepdim=True).sqrt() + 1e-12)
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (
                                ((x_adv_1 - x) ** 2).sum(
                                    dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12
                    ) * torch.min(self.eps * torch.ones(x.shape,
                                                        device=x.device
                                                        ).detach(),
                                  ((x_adv_1 - x) ** 2).sum(
                            dim=(1, 2, 3), keepdim=True).sqrt()), 0.0, 1.0)
            ...
            with torch.no_grad():
                ...
                if counter3 == k:
                    ...
                    if np.sum(fl_oscillation) > 0:
                        ...
                        grad[fl_oscillation] = grad_best[fl_oscillation
                                                         ].clone()

...
from .utils import project_perturbation, normalize_perturbation, \
    create_early_stopping_mask, initialize_perturbation
...
class ArgminPGD(RestartAttack):
    ...
    def perturb_inner(self, x, y, targeted=False, x_init=None):
        ...
        pert = initialize_perturbation(x, self.eps, self.norm, x_init,
            ...
            with torch.no_grad():
                ...
                if self.normalize_grad:
                    ...
                    l1_norm_gradient = 1e-10 + torch.sum(
                        grad.abs().reshape(x.shape[0], -1),
                        dim=1).view(-1, 1, 1, 1)
                    velocity = self.momentum * velocity + \
                        grad / l1_norm_gradient

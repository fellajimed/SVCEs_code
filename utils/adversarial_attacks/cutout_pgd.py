...
class CutoutPGD(RestartAttack):
    def __init__(self, eps, iterations, stepsize, num_classes, mask_size=16,
                 momentum=0.9, decay=1.0, norm='inf', loss='CrossEntropy',
                 normalize_grad=False, early_stopping=0, restarts=0,
                 init_noise_generator=None, model=None, save_trajectory=False):
        super().__init__(loss, restarts, num_classes, model=model,
                         save_trajectory=save_trajectory)
        # loss either pass 'CrossEntropy', 'LogitsDiff' or custom loss function
        self.eps = eps
    ...
    def perturb_inner(self, x, y, targeted=False, x_init=None):
        ...
        if self.save_trajectory:
            trajectory = torch.zeros((self.iterations + 1,) + x.shape,
                                     device=x.device)
            trajectory[0, :] = x
        else:
            trajectory = None
        ...
        for i in range(self.iterations + 1):
            ...
            with torch.enable_grad():
                ...
                best_losses[new_best] = loss_expanded[new_best
                                                      ].clone().detach()
            ...
            with torch.no_grad():
                ...
                if self.normalize_grad:
                    # https://arxiv.org/pdf/1710.06081.pdf the l1
                    # normalization follows the momentum iterative method
                    l1_norm_gradient = 1e-10 + torch.sum(
                        grad.abs().view(x.shape[0], -1),
                        dim=1).view(-1, 1, 1, 1)
                    velocity = self.momentum * velocity + grad
                    velocity /= l1_norm_gradient
                    norm_velocity = normalize_perturbation(velocity, self.norm)
                else:
                    # velocity update as in pytorch
                    # https://pytorch.org/docs/stable/optim.html
                    velocity = self.momentum * velocity + grad
                    norm_velocity = velocity

                pert = pert - (self.decay**i) * self.stepsize * \
                    mask * norm_velocity
                # todo check order

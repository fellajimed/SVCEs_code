...
class SSIMDistance(Distance):
    def __init__(self, kernel_w=3, sigma=1.5, channels=1):
        ...
        self.ssim_d = SSIM(win_size=kernel_w, win_sigma=sigma, data_range=1.0,
                           size_average=False, channel=channels)
        self.config = {'Distance': 'SSIM', 'win_size': kernel_w,
                       'win_sigma': sigma}
    ...
    def dist(self, X, Y, *args, **kwargs):
        ...
        N = X.shape[0]
        d1 = torch.clamp_min(
            1 - (self.ssim_d(X, Y)).view(N, -1).mean(dim=1), 0.0)
        return d1
...
class MSSSIMDistance(Distance):
    def __init__(self, kernel_w=3, sigma=1.5, channels=1, weights=None):
        ...
        self.ssim_d = MS_SSIM(win_size=kernel_w, win_sigma=sigma,
                              data_range=1.0, channel=channels,
                              weights=self.weights, size_average=False)
...
class ReconstructionLoss(Distance):
    def __init__(self, alpha=0.84, kernel_w=3, sigma=1.5, channels=1):
        ...
        self.ssim_d = SSIM(win_size=kernel_w, win_sigma=sigma, data_range=1.0,
                           size_average=False, channel=channels)
        self.config = {'Distance': 'L1 + SSIM', 'alpha': alpha,
...
class MSReconstructionLoss(Distance):
    def __init__(self, alpha=0.84, kernel_w=3, sigma=1.5,
                 channels=1, weights=None):
        super().__init__()
        self.alpha = alpha
        self.kernel_w = kernel_w
        self.sigma = sigma
    ...
    def dist(self, X, Y, *args, **kwargs):
        ...
        d1 = torch.clamp_min(ms_ssim(X, Y, win_size=self.kernel_w,
                                     win_sigma=self.sigma, data_range=1.0,
                                     size_average=False, weights=weights), 0.0)

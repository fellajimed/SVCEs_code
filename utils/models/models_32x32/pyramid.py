...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
                 downsample=None, p_shakedrop=1.0):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
        ...
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, (planes * 1), kernel_size=3,
                               stride=stride, padding=1, bias=False)
...
class aa_PyramidNet(nn.Module):
    def __init__(self, dataset='cifar10', depth=272, alpha=200,
                 num_classes=10, bottleneck=True, is_rgb=True):
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.ps_shakedrop = [1. - (1.0 - (0.5 / (3 * n)) * (i + 1))
                                 for i in range(3 * n)]

            self.input_featuremap_dim = self.inplanes
            self.conv1 = nn.Conv2d(1+2*int(is_rgb), self.input_featuremap_dim,
    ...
    def pyramidal_make_layer(self, block, block_depth, stride=1):
        ...
        layers.append(block(self.input_featuremap_dim,
                            int(round(self.featuremap_dim)), stride,
                            downsample, p_shakedrop=self.ps_shakedrop.pop(0)))

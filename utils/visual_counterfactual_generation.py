...
def _prepare_targeted_translations(model_descriptions, imgs, target_list,
                                   class_labels, device, dataset, bs,
                                   device_ids=None):
    num_classes = len(class_labels)
    num_datapoints = len(imgs)
    num_models = len(model_descriptions)
    probabilities = torch.zeros((num_datapoints, len(model_descriptions),
                                 num_classes), dtype=torch.float32)

    for model_idx in trange(num_models, desc='Models progress'):
        type, folder, checkpoint, temperature, temp = \
            model_descriptions[model_idx]
        model = load_model(type, folder, checkpoint, temperature, device,
                           load_temp=temp, dataset=dataset)
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(n_batches):
                ...
                probabilities[datapoint_idx:datapoint_end_idx, model_idx] = \
    ...
    all_models_perturbation_targets = torch.zeros((num_datapoints, num_models,
                                                   max_num_targets),
                                                  dtype=torch.long)
...
def _find_wrong_examples(model_descriptions, dataloader, num_examples,
                         class_labels, device, dataset, device_ids=None):
    num_classes = len(class_labels)
    ...
    predictions = torch.zeros((num_datapoints, len(model_descriptions)),
                              dtype=torch.long)
    probabilities = torch.zeros((num_datapoints, len(model_descriptions),
                                 num_classes), dtype=torch.float32)
    failure = torch.zeros((num_datapoints, len(model_descriptions)),
                          dtype=torch.bool)
    ...
    with torch.no_grad():
        for model_idx in trange(num_models, desc='Models progress'):
            type, folder, checkpoint, temperature, temp = \
                model_descriptions[model_idx]
            model = load_model(type, folder, checkpoint, temperature,
                               device, load_temp=temp, dataset=dataset)
            # search failure cases
            if device_ids is not None and len(device_ids) > 1:
                model = nn.DataParallel(model, device_ids=device_ids)
            model = TemperatureWrapper(model, temperature)
            ...
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(dataloader):
                    if model_idx == 0:
                        imgs[datapoint_idx:(datapoint_idx+data.shape[0]),
                             :] = data
                        targets[datapoint_idx:(datapoint_idx + data.shape[0])
                                ] = target
                    ...
                    predictions[datapoint_idx:(datapoint_idx+data.shape[0]),
                                model_idx] = orig_pred.detach().cpu()
                    probabilities[datapoint_idx:(datapoint_idx+data.shape[0]),
                                  model_idx] = orig_confidences.detach().cpu()
                    failure[datapoint_idx:(datapoint_idx+data.shape[0]),
                            model_idx] = ~correct

                    datapoint_idx += data.shape[0]

        all_model_failure = (torch.sum(failure, dim=1) >=
                             len(model_descriptions))
        all_model_failure_idcs = torch.nonzero(all_model_failure,
        ...
        all_models_original_probabilities = \
            probabilities[all_model_failure_idcs, :]

        perturbation_targets = torch.zeros((examples_found, num_models, 2),
                                           dtype=torch.long)
        for i in range(num_models):
            perturbation_targets[:, i, 0] = all_models_targets
            perturbation_targets[:, i, 1] = \
                predictions[all_model_failure_idcs, i]

        print(f'Found {examples_found} out of {num_examples}',
...
def _plot_counterfactuals(dir, model_name, model_checkpoint, original_imgs,
                          original_probabilities, targets, perturbed_imgs,
                          perturbed_probabilities, radii, class_labels,
                          filenames=None, plot_single_images=False,
                          show_distances=False):

    num_imgs = targets.shape[0]
    num_radii = len(radii)
    scale_factor = 1.5
    for img_idx in trange(
            num_imgs, desc=f'{model_name} {model_checkpoint} - Image write'):
        if filenames is None:
            single_img_dir = os.path.join(dir, f'{img_idx}')
        else:
            single_img_dir = os.path.join(
                dir, os.path.splitext(filenames[img_idx])[0])

        if plot_single_images:
            pathlib.Path(single_img_dir).mkdir(parents=True, exist_ok=True)

        img_targets = targets[img_idx, :]
        valid_target_idcs = torch.nonzero(img_targets != -1,
        ...
        fig, ax = plt.subplots(num_rows, num_cols,
                               figsize=(scale_factor * num_cols,
                                        num_rows * 1.3 * scale_factor))
        if num_rows == 1:
            ax = np.expand_dims(ax, 0)
        img_original = original_imgs[img_idx,
                                     :].permute(1, 2, 0).cpu().detach()
        img_probabilities = original_probabilities[img_idx, :]
        img_confidence, img_prediction = torch.max(img_probabilities, dim=0)

        if plot_single_images:
            _plot_single_img(img_original,
                             os.path.join(single_img_dir, 'original.png'))

        if num_targets == 2:
            title = f'{class_labels[img_targets[0]]}:' + \
                f' {img_probabilities[img_targets[0]]:.2f}\n' + \
                f'{class_labels[img_targets[1]]}:' + \
                f' {img_probabilities[img_targets[1]]:.2f}'
        else:
        ...
        for target_idx_idx in range(num_targets):
            ...
            for radius_idx in range(len(radii)):
                img = torch.clamp(
                    perturbed_imgs[img_idx, target_idx, radius_idx
                                   ].permute(1, 2, 0), min=0.0, max=1.0)
                img_target = targets[img_idx, target_idx]
                img_probabilities = perturbed_probabilities[img_idx,
                                                            target_idx,
                                                            radius_idx]
                ...
                ax[target_idx, radius_idx + 1].imshow(img,
                ...
                if plot_single_images:
                    _image = f'target_{target_idx}_radius_{radius_idx}'
                    _plot_single_img(
                        img, os.path.join(single_img_dir, f'{_image}.png'))
                    _plot_diff_image(
                        img_original, img,
                        os.path.join(single_img_dir, f'{_image}_diff.png'))
...
def _inner_generation(original_imgs, perturbation_targets,
                      all_model_original_probabilities, model_descriptions,
                      radii, bs, class_labels, device, eval_dir, dataset,
                      norm, steps, stepsize, attack_type, filenames=None,
                      plot_single_images=False, show_distanes=False,
                      device_ids=None, verbose=True):
    num_classes = len(class_labels)
    ...
    with torch.no_grad():
        ...
        for model_idx in range(len(model_descriptions)):
            ...
            model = load_model(type, model_folder, model_checkpoint,
                               temperature, device, load_temp=temp,
                               dataset=dataset)
            if device_ids is not None and len(device_ids) > 1:
                model = nn.DataParallel(model, device_ids=device_ids)
            model = TemperatureWrapper(model, temperature)
            ...
            out_imgs = torch.zeros((num_imgs, num_targets, num_radii) +
                                   img_dimensions)
            out_probabilities = torch.zeros((num_imgs, num_targets,
                                             num_radii, num_classes))

            n_batches = int(np.ceil(num_imgs / model_bs))

            for batch_idx in trange(n_batches, desc=f'{model_folder}' +
                                    f' {model_checkpoint} - Batches progress'):
                sleep(0.1)
                ...
                batch_targets = perturbation_targets[
                    batch_start_idx:batch_end_idx, model_idx]

                for radius_idx in range(len(radii)):
                    eps = radii[radius_idx]

                    if temperature is not None and temperature < 0.2:
                        # 'conf' #'logitsdiff'#'logit_max_target'
                        loss = 'ce-targeted-cfts-conf' \
                            if norm.lower() in ['l1', 'l2'] else 'log_conf'
                        raise NotImplementedError()
                    else:
                        # 'conf' #'logitsdiff'#'conf'
                        loss = 'ce-targeted-cfts-conf' \
                            if norm.lower() in ['l1', 'l2'] else 'log_conf'
                    if verbose:
                    ...
                    attack_config = create_attack_config(
                        eps, steps, stepsize, norm, pgd=attack_type,
                        normalize_gradient=True)
                    att = get_adversarial_attack(
                        attack_config, model, loss, num_classes)
                    ...
                    for target_idx in range(num_targets):
                        ...
                        num_valid_batch_targets = \
                            torch.sum(valid_batch_targets).item()
                        batch_adv_samples_i = torch.zeros_like(batch_data)
                        if num_valid_batch_targets > 0:
                            if norm.lower() in ['l1', 'l2']:
                                batch_valid_adv_samples_i = \
                                    att.perturb(
                                        batch_data[valid_batch_targets],
                            ...
                            batch_adv_samples_i[valid_batch_targets] = \
                                batch_valid_adv_samples_i
                            ...
                            if verbose:
                                ...
                                print('batch targets are',
                                      batch_targets_i[valid_batch_targets])
                                print('final probs are',
                                      batch_probs_i.gather(1, batch_targets_i[
                                          valid_batch_targets].reshape(-1, 1)))
                            out_imgs[batch_start_idx:batch_end_idx,
                                     target_idx, radius_idx, :] = \
                                batch_adv_samples_i.cpu().detach()
                            out_probabilities[batch_start_idx:batch_end_idx,
                                              target_idx, radius_idx, :] = \
                                batch_probs_i.cpu().detach()

            model_original_probabilities = \
                all_model_original_probabilities[:, model_idx]

            if verbose:
                print(f'min: {torch.min(out_imgs).item()}',
                      f'- max {torch.max(out_imgs).item()}')

            _plot_counterfactuals(dir, model_folder, model_checkpoint,
                                  original_imgs, model_original_probabilities,
                                  perturbation_targets[:, model_idx, :],
                                  out_imgs, out_probabilities, radii,
                                  class_labels, filenames=filenames,
                                  plot_single_images=plot_single_images,
                                  show_distances=show_distanes)

            # out_dict = {'model_original_probabilities':
            #             model_original_probabilities,
...
# looks through dataloader to find examples that are misclassified
# by all models,
# then for each create a counterfactual in the correct and wrong class
def visual_counterfactuals(model_descriptions, radii, dataloader, bs,
                           num_examples, class_labels, device, eval_dir,
                           dataset, norm='L2', steps=500, stepsize=0.1,
                           attack_type='apgd', device_ids=None):
    original_imgs, perturbation_targets, all_models_original_probabilities = \
        _find_wrong_examples(model_descriptions, dataloader, num_examples,
                             class_labels, device, dataset,
                             device_ids=device_ids)

    _inner_generation(original_imgs, perturbation_targets,
                      all_models_original_probabilities, model_descriptions,
                      radii, bs, class_labels, device, eval_dir, dataset,
                      norm, steps, stepsize, attack_type,
                      device_ids=device_ids)


# create counterfactuals for all models on all datapoints [N, 3, IMG_W, IMG_H]
# in the given classes
# target list should be a nested list where
def targeted_translations(model_descriptions, radii, imgs, target_list, bs,
                          class_labels, device, eval_dir, dataset, norm='L2',
                          steps=500, stepsize=0.1, attack_type='apgd',
                          show_distanes=False, filenames=None,
                          device_ids=None):
    ...
    _inner_generation(
        imgs, perturbation_targets, all_models_original_probabilities,
        model_descriptions, radii, bs, class_labels, device, eval_dir,
        dataset, norm, steps, stepsize, attack_type, filenames=filenames,
        plot_single_images=True, show_distanes=show_distanes,
        device_ids=device_ids)
